...
const sqlPath = 'd:/Code/XuatLe/xuatle_stored.sql';
let sql = fs.readFileSync(sqlPath, 'utf8');

// 1. Replace SaveDraft_Json
const saveDraftRegex = /CREATE OR ALTER PROC \[dbo\]\.\[usp_XuatLe_YeuCau_SaveDraft_Json\].*?END\r?\nGO/s;
...
END
GO`;
sql = sql.replace(saveDraftRegex, newSaveDraft);


// 2. Replace GetByID
const getByIdRegex = /CREATE OR ALTER PROC \[dbo\]\.\[usp_XuatLe_YeuCau_GetByID\].*?END\r?\nGO/s;
...
END
GO`;
sql = sql.replace(getByIdRegex, newGetById);


// 3. Replace GetList
const getListRegex = /CREATE OR ALTER PROC \[dbo\]\.\[usp_XuatLe_YeuCau_GetList\].*?END\r?\nGO/s;
...
END
GO`;
sql = sql.replace(getListRegex, newGetList);

fs.writeFileSync(sqlPath, sql);
console.log('xuatle_stored.sql updated!');
